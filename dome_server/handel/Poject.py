...
class Poject(Basehandelr):
    ...
    def get_list(self):
        ...
        try:
            if key:
                coures = self.cooliect.find({"Adminid":Adminid,"findtime":{"$gt":start,"$lt":end},"timeend":{"$in":times},"titile":{"$regex":key}}).limit(settings.PAGE_NUM).skip(settings.PAGE_NUM * (page - 1)).sort([("createtime", -1)])
            else:
                coures=self.cooliect.find({"Adminid":Adminid,"findtime":{"$gt":start,"$lt":end},"timeend":{"$in":times}}).limit(settings.PAGE_NUM).skip(settings.PAGE_NUM*(page-1)).sort([("createtime",-1)])
            count=coures.count()
            for i in coures:
                data={}
